...
象你看一的，箭头函数在这种情况下可节省你敲括号,'function'和'return'等关键字的时间。建议不论什么情况下总是用括号括住参数，因为括号里可以写多个参数，如`(x,y) => x+y`。这种作法可以让我们忽略它在不同场景下的不同的法，虽然上面的代码这样写也可以：`x => x*x`。目前为止我们涉及的都在语法层面，它让代码更简洁，更易读。



### this绑定

使用胖箭头函数还有一个很好原因。在JS中, `this`一直就是容易出错的地方。使用箭头函数，你再也不用担心`.bind(this)`或者设置`that=this`了，因为箭头函数根据上下文环境自动选择了`this`对象。看下这个[例子](https://jsfiddle.net/pklinger/rw94oc11/)：
